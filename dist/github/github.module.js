"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.GithubModule = void 0;
const common_1 = require("@nestjs/common");
const config_1 = require("@nestjs/config");
const nestjs_github_webhooks_1 = require("@dev-thought/nestjs-github-webhooks");
const github_controller_1 = require("./github.controller");
const github_service_1 = require("./github.service");
const github_app_service_1 = require("./github-app.service");
let GithubModule = class GithubModule {
};
exports.GithubModule = GithubModule;
exports.GithubModule = GithubModule = __decorate([
    (0, common_1.Module)({
        imports: [
            nestjs_github_webhooks_1.GithubWebhooksModule.forRootAsync({
                inject: [config_1.ConfigService],
                useFactory: (config) => ({
                    webhookSecret: config.getOrThrow('GITHUB_WEBHOOK_SECRET'),
                }),
            }),
        ],
        controllers: [github_controller_1.WebhookController],
        providers: [github_service_1.GithubService, github_app_service_1.GithubAppService],
        exports: [github_app_service_1.GithubAppService, github_service_1.GithubService],
    })
], GithubModule);
//# sourceMappingURL=github.module.js.map